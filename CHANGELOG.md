...
- Move stack service configuration from TOML files to normalized SQLite tables and rework the Config page to edit services through per-field widgets — with dropdown-driven dependency/trigger chains and a TOML Export/Import pair for moving config in and out as text — instead of a raw TOML textarea; upgrading a stack still on file-based config needs one manual Import, see README ([#163](https://github.com/NullNet-ai/nullnet/pull/163) — part of [#140](https://github.com/NullNet-ai/nullnet/issues/140))
