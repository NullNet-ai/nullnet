...
## [UNRELEASED]
### Added
- Per-service egress/ingress traffic filters: arbitrary AND/OR/group combinations of Country, ASN, Src IP (ingress), and Dst IP (egress) conditions, evaluated via `rpn-predicate-interpreter` — replaces the country-only egress/ingress policy (fixes [#143](https://github.com/NullNet-ai/nullnet/issues/143))
### Changed
