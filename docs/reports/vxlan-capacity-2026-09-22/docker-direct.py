...
def setup(i):
 ...
 mark('forwarding')
 return {'i':i,'docker_container':container,'docker_pid':pid,'seconds':time.monotonic()-begin,'stages':stages}
if __name__=='__main__':
 ...
 result={'endpoint_type':'docker' if os.environ.get('NN_DOCKER_PREFIX') else 'standalone','forwarding_once':os.environ.get('NN_FORWARD_ONCE')=='1','sysctl_per_endpoint':True,'pid_cached':os.environ.get('NN_PID_CACHE')=='1','inventory':inventory,'n':N,'workers':WORKERS,'seconds':elapsed,'success':len(metrics),'errors':errors,'endpoints':metrics}
