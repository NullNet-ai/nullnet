...
for h in ['104']:
 rows=[]
 for line in (P/f'bare-kernel-{h}.trace').read_text().splitlines():
  m=rx.match(line)
  if not m:continue
 ...
 (P/f'bare-kernel-{h}-parsed.json').write_text(json.dumps(rows))
 (P/f'bare-kernel-{h}-summary.json').write_text(json.dumps(summary,indent=2))
