...
if __name__=='__main__':
 ...
 # Capture normalized configuration after the timer, using only dummy test keys.
 if os.environ.get('NN_FINGERPRINT')=='1':
  import re,hashlib
  config={}
  links=json.loads(subprocess.check_output(['ip','-d','-j','link','show'],text=True))
  config['host_links']=[{k:v for k,v in x.items() if k in ['ifname','mtu','master','flags','linkinfo']} for x in links if x['ifname'].startswith(('vd','bd','xd'))]
  config['host_addresses']=[{'ifname':x['ifname'],'addr_info':x.get('addr_info',[])} for x in json.loads(subprocess.check_output(['ip','-j','addr'],text=True)) if x['ifname'].startswith(('vd','bd','xd'))]
  config['containers']={}
  for name,pid in PID_CACHE.items():
   links=json.loads(subprocess.check_output(['nsenter','-t',str(pid),'-n','ip','-j','addr'],text=True))
   config['containers'][name]=[{k:v for k,v in x.items() if k in ['ifname','mtu','flags','addr_info']} for x in links if x['ifname'].startswith('vd')]
  for kind in ['state','policy']:
   raw=subprocess.check_output(['ip','xfrm',kind],text=True)
   # Allocated policy indexes differ between otherwise identical namespace incarnations.
   config[kind]=re.sub(r'index [0-9]+','index <allocated>',raw)
  for values in [config['host_addresses']]+list(config['containers'].values()):
   for link in values:link['addr_info']=[a for a in link.get('addr_info',[]) if a['family']=='inet']
  pathlib.Path('/tmp/nn-config.json').write_text(json.dumps(config,sort_keys=True))
