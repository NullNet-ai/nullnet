...
def bench(w,s,n,phase,repeat=0):
 ...
 results.append(r);pathlib.Path('/tmp/nn-refine-results.json').write_text(json.dumps(results))
...
try:
 ...
 grid=[(w,s) for w in [1,2,4,16] for s in [1,2,4,8]]+[(8,1),(8,2)];random.Random(22).shuffle(grid)
 for w,s in grid:bench(w,s,256,'coarse')
 best=[];seen=set()
 for r in sorted(results,key=lambda r:r['rate'],reverse=True):
  pair=(r['workers'],min(r['workers'],r['slots']))
  if pair not in seen:best.append(r);seen.add(pair)
  if len(best)==2:break
 pairs=list(dict.fromkeys([(8,8)]+[(r['workers'],r['slots']) for r in best]))
